...
class Expenses(Document):
	...
	def from_journal_entry(self, journal_entry):
		...
		self.posting_date = getattr(journal_entry, "cheque_date", self.posting_date)
		...
		if len(accounts) > 0:
			...
			self.amount = getattr(accounts[0], "credit_in_account_currency", None)
		if len(accounts) > 1:
			self.debit_account = getattr(accounts[1], "account", None)
			# إذا لم يتم تعيين المبلغ من الصف الأول، خذ من الصف الثاني
			if not self.amount:
				self.amount = getattr(accounts[1], "debit_in_account_currency", None)
